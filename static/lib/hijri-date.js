...
function HijriDate() {
    let hd = typeof this == 'object' ? this : window, time, tzom = Date.parse('01 Jan 1970'), tzo = parseInt(parseInt(tzom / 1000) / 60),
        tzs = Date(1970, 0, 1),
    ...
    hd.toISOString = function () {
        let d = HijriDate.toNDigit;
        return d(utc.yyy, utc.yyy < 0 ? 6 :
            4) + '-' + d(utc.mmm + 1, 2) + '-' + d(utc.ddd, 2) + 'T' + getUTCTmStr() + '.' + d(utc.ms, 3) + 'Z';
    };
    hd.toJSON = function () {
...
HijriDate.monthNames =
    ['Muharram', 'Safar', 'Rabi\'ul-Awwal', 'Rabi\'ul-Akhir', 'Jumadal-Ula', 'Jumadal-Akhir', 'Rajab', 'Sha\'ban', 'Ramadan', 'Syawwal',
